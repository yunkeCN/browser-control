...
支持的浏览器命令包括 `navigate`、`find_tab`、`snapshot`、`get_text`、`scroll`、`click`、`click_probe`、`fill`、`press`、`select_option`、`set_checked`、`wait_for`、`evaluate`、`screenshot`、`save_as_pdf`、`observe_start`、`observe_diff`、`network_start`、`network_list`、`network_detail`、`network_stop`、`upload`、`download`、`list_tabs`、`close_tab` 和 `close_session`。
...
- `browser-control-skill-<tag>.zip`：完整的 `browser-control/` Skill 目录，包含 `SKILL.md`、`scripts/`、`references/`、`extension/` 和已打包的 daemon runtime。
...
生成后的 Skill 产物会提交在 `skills/browser-control/`：可加载的扩展位于 `extension/`，生成后的 protocol/daemon runtime 位于 `scripts/`。MCP runtime 生成在 `bin/browser-control-mcp.mjs`。
