...
- `<browser-command> [--session <name>] [--args <json>] [--args-file <path>] [--code-file <path>] [--timeout-ms <ms>] [--id <id>]`: send a browser command envelope to `POST /command`. Browser commands are `navigate`, `snapshot`, `click`, `fill`, `press`, `scroll`, `upload`, `get_text`, `capture`, `evaluate`, `wait_for`, `network`, `tabs`, `close_session`, and `download`.
- `e2e --help`: print live end-to-end test handoff instructions.

Browser command helper examples:

```bash
node skills/browser-control/scripts/browser-control.js snapshot --session demo --args '{}'
node skills/browser-control/scripts/browser-control.js click --session demo --args '{"target":"@e1jm0sbb_1"}'
node skills/browser-control/scripts/browser-control.js click --session demo --args '{"target":"@e1jm0sbb_1","interceptRequests":{"filter":"/api/"}}'
node skills/browser-control/scripts/browser-control.js scroll --session demo --args '{"deltaY":800,"strategy":"dom"}'
node skills/browser-control/scripts/browser-control.js evaluate --session demo --code-file ./snippet.js
node skills/browser-control/scripts/browser-control.js tabs --session demo --args '{"action":"list"}'
node skills/browser-control/scripts/browser-control.js capture --session demo --args '{"format":"png"}'
```

Use `--args-file` for large JSON argument objects. Use `--code-file` only with `evaluate`; it reads UTF-8 JavaScript into `args.code` and avoids shell/JSON escaping. `--session`, `--timeout-ms`, and `--id` set envelope fields and override matching values from `--args` / `--args-file`. Browser Control intentionally does not support `codeBase64`.
...
```bash
node skills/browser-control/scripts/browser-control.js snapshot --session demo --args '{"viewportOnly":true,"hasVisibleText":true}'
node skills/browser-control/scripts/browser-control.js snapshot --session demo --args '{"roles":["button","link","textbox","combobox"],"viewportOnly":true}'
node skills/browser-control/scripts/browser-control.js snapshot --session demo --args '{"diff_to":"page-1"}'   # first call stores baseline
node skills/browser-control/scripts/browser-control.js snapshot --session demo --args '{"diff_to":"page-1"}'   # second call returns diff
```
