...
2. Navigate or find an existing tab:
   ```bash
   node scripts/browser-control.js navigate --session read-page --args '{"url":"https://example.com","newTab":true}'
   ```
3. Capture compact page structure for targets and layout context:
   ```bash
   node scripts/browser-control.js snapshot --session read-page --args '{}'
   ```
   For noisy pages, prefer snapshot filters before reading a broad structure snapshot:
   ```bash
   node scripts/browser-control.js snapshot --session read-page --args '{"hasVisibleText":true,"viewportOnly":true}'
   ```
...
4. If you only need visible text, use `get_text` instead of custom JavaScript or a broad snapshot:
   ```bash
   node scripts/browser-control.js get_text --session read-page --args '{"scope":"viewport","maxChars":4000}'
   ```
5. For below-fold content or scrollable result regions, use first-class `scroll` rather than keyboard keys or an `evaluate` workaround, then read text again:
   ```bash
   node scripts/browser-control.js scroll --session read-page --args '{"deltaY":800,"strategy":"dom"}'
   node scripts/browser-control.js scroll --session read-page --args '{"strategy":"wheel","x":400,"y":500,"deltaY":800}'
   node scripts/browser-control.js get_text --session read-page --args '{"scope":"full","maxChars":12000,"includeRuns":true}'
   ```
...
3. Perform the action:
   ```bash
   node scripts/browser-control.js click --session ui-task --args '{"target":"@e1jm0sbb_1"}'
   ```
...
2. Fill fields:
   ```bash
   node scripts/browser-control.js fill --session form-draft --args '{"target":"@e0field_1","value":"example text"}'
   node scripts/browser-control.js click --session form-draft --args '{"target":"@e0select_1"}'
   node scripts/browser-control.js press --session form-draft --args '{"key":"ArrowDown"}'
   node scripts/browser-control.js press --session form-draft --args '{"key":"Enter"}'
   node scripts/browser-control.js click --session form-draft --args '{"target":"@e1jm0sbb_1"}'
   ```
...
4. Snapshot, click the final control by `@e`, and verify result:
   ```bash
   node scripts/browser-control.js click --session submit-task --args '{"target":"@e0submit_1"}'
   node scripts/browser-control.js wait_for --session submit-task --args '{"text":"Success","timeoutMs":10000}'
   ```
...
2. Call `capture` with `format: "pdf"`:
   ```bash
   node scripts/browser-control.js capture --session pdf-task --args '{"format":"pdf","fileName":"page.pdf","paperFormat":"A4","printBackground":true}'
   ```
...
1. Navigate or perform the triggering UI action:
   ```bash
   node scripts/browser-control.js navigate --session net-task --args '{"url":"https://example.com","newTab":true}'
   ```
2. List captured requests:
   ```bash
   node scripts/browser-control.js network --session net-task --args '{"action":"list"}'
   ```
3. Fetch details for relevant `requestId` values:
   ```bash
   node scripts/browser-control.js network --session net-task --args '{"action":"detail","requestId":"<id>"}'
   ```
...
1. Navigate or directly call download when a URL is known:
   ```bash
   node scripts/browser-control.js download --session dl-task --args '{"url":"https://example.com/file.csv","filename":"file.csv"}'
   ```
...
```bash
node scripts/browser-control.js close_session --session demo --args '{}'
...
```bash
node scripts/browser-control.js evaluate --session js-task --code-file ./snippet.js
```
