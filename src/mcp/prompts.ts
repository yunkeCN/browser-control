...
export function registerBrowserControlPrompts(server: McpServer): void {
  server.registerPrompt('browser_control_usage', {
    ...
    '',
    'DOM structure diff: every snapshot automatically saves with a baselineId (returned in the response). Pass diff_to=<previousId> to compare the current snapshot against a historical one — returns structured added/removed/changed subtrees. click also auto-creates a baseline and returns observationBaselineId.',
    '',
  ...
  server.registerPrompt('browser_control_command_reference', {
    ...
    return textPrompt('Browser Control command reference', [
      ...
      '- click captures network requests triggered within 700ms (URLs only, no body). For advanced network capture (headers/body, URL filtering), use click_probe.',
      ...
      '{"command":"snapshot","args":{"diff_to":"obs1"}} — compare current snapshot against baseline obs1, returns structured added/removed/changed diff',
      '{"command":"click","args":{"target":"@eabc_1"}}',
