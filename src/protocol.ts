...
export interface ProtocolModule {
  ...
  DAEMON_CAPABILITIES: string[];
  EXTENSION_CAPABILITY_HINTS: string[];
  COMMANDS: Record<string, CommandSpec>;
}

export const PROTOCOL_VERSION = '2026-05-19';
export const RUNTIME_SCHEMA_VERSION = '2026-05-20';
...
const ELEMENT_REF_PATTERN = /^@e[^\s_]+_\d+$/;
const TARGET_COMMANDS = new Set(['click', 'click_probe', 'fill', 'press', 'scroll', 'upload']);
const LEGACY_TARGET_ARGS = ['elementRef', 'selector'];
...
export class ProtocolError extends Error {
  ...
  constructor(code: string, message: string, details: unknown = undefined, retryable = false) {
    super(message);
...
function validateKnownArgs(request: any, spec: CommandSpec) {
...
function validateRequiredOneOf(request: any, spec: CommandSpec) {
  const fields = spec.requiredOneOf || [];
  if (!fields.length) return;
  if (fields.some(field => request.args[field] !== undefined && request.args[field] !== null && request.args[field] !== '')) return;
...
function nearestArgName(field: string, candidates: string[]): string | null {
  let best = null;
...
function validateOptionalStringArg(request: any, spec: CommandSpec, field: string, commands: string[]) {
...
function validateOptionalBooleanArgs(request: any, fields: string[]) {
...
function validateOptionalNumberArgs(request: any, fields: string[]) {
  for (const field of fields) {
    if (!Object.prototype.hasOwnProperty.call(request.args || {}, field)) continue;
    const value = request.args[field];
...
function validationDetails(request: any, spec: CommandSpec, field: string): any {
  const details: any = {
    field,
    command: request.command,
    required: spec.required,
...
export function protocolErrorFrom(err: any, fallbackCode = 'BACKEND_ERROR') {
...
export function createResultEnvelope(request: any, { ok, backend = 'extension', tab = null, data = null, artifacts = [], error = null, diagnostics = null, startedAt, endedAt = new Date().toISOString() }: any) {
...
function validateTargetArg(request: any, spec: CommandSpec) {
...
function mapTargetArgs(args: any) {
  const selector = targetToSelector(args?.target);
  return selector ? { ...args, selector } : args;
}

function targetToSelector(target: unknown): string | null {
